...
class QueryParamsTest {
    ...
    @Test
    fun `encode percent-encodes names and values with utf8`() {
        val params =
            QueryParams.builder()
                .add("q", "a b&c")
                .add("name+key", "x")
                .build()
        assertEquals("q=a+b%26c&name%2Bkey=x", params.encode())
    }
    ...
    fun `parse decodes names and values`() {
        val params = QueryParams.parse("q=a+b%26c&page=2")
    ...
    fun `encode then parse round-trips names values and order`() {
        ...
        assertEquals(original.names().toList(), roundTripped.names().toList())
        assertEquals(original.values("q"), roundTripped.values("q"))
        assertEquals(original.values("tag"), roundTripped.values("tag"))
        assertEquals(original.values("special"), roundTripped.values("special"))
    }
