 ...
 * - the current page's items are exhausted AND `page.nextPageRequest()` returned `null`.
 *
 * The two conditions are semantically distinct but produce the same outcome: end of stream.
 *
 * ## Response lifecycle
 *
...
public class Paginator<T>(
    ...
    public fun iterateAll(): Iterable<T> =
        Iterable {
            PaginatorIterator(httpClient, initialRequest, strategy)
        }
     ...
     * the page reports `hasNext` and produces a non-null next request.
     */
