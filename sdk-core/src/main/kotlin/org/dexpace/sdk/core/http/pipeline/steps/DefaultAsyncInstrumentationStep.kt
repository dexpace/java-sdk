...
public class DefaultAsyncInstrumentationStep @JvmOverloads constructor(
    ...
    override fun processAsync(request: Request, next: AsyncPipelineNext): CompletableFuture<Response> {
        ...
        val span = options.tracer.startSpan(
        ...
        val downstream: CompletableFuture<Response> = span.makeCurrentWithLoggingContext().use {
            ...
            try {
                ...
                recordMetrics(request, statusCode = -1, elapsedMs, errorType = e::class.java.simpleName ?: "Throwable")
                span.end(e)
    ...
    private fun buildOutgoingRequest(request: Request): Pair<Request, LoggableRequestBody?> {
        ...
        val wrappedRequestBody = if (shouldCaptureBody() && requestBody != null) {
            LoggableRequestBody(requestBody)
        } else null
        val outgoing = if (wrappedRequestBody != null) {
     ...
     * [processAsync] within the line-count budget.
     */
    ...
    private fun emitRequestEvent(request: Request, redactedUrl: String) {
    ...
    private fun spanAttributes(request: Request, redactedUrl: String): Map<String, Any> =
        mapOf(
            "http.request.method" to request.method.name,
            "url.full" to redactedUrl,
        )

    private fun recordMetrics(request: Request, statusCode: Int, elapsedMs: Double, errorType: String?) {
        val attrs: Map<String, Any> = if (errorType != null) {
            mapOf(
                "http.request.method" to request.method.name,
    ...
    private fun elapsedMillis(startNanos: Long): Double =
        (clock.monotonic() - startNanos) / 1_000_000.0
